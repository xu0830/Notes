...
Mysql 索引使用的是 B+ 树，因为索引时用来加快查询的，而 B+ 树通过对数据进行排序，所以可以提高查询速度，然后通过一个节点中可以存储多个元素，从而可以使得 B+ 树的高度不会太高，在Mysql 中一个 InnoDB 页也就是一个 B+树节点，一个 InnoDB 默认 16KB,所以一般情况下一棵两层的 B+ 树可以存放 2000万左右的数据，然后通过利用 B+树叶子节点存储了所有数据并且进行了排序，并且叶子节点之间有指针，可以很快的支撑全表扫描，范围查找等 SQL 语句
